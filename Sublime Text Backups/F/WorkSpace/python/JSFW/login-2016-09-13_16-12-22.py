...
class Login:
    ...
    def Login(self,uname,pw):
        ...
        time.sleep(3)
        Mytool.findCss(dr,"button[class='s_btn_logins']").click()
#选择合同有效期
    def duringDate(self,st,ed):
        driver=self.driver
        t1='document.getElementById("subs_time_s").readOnly=false'
        t2='document.getElementById("subs_time_e").readOnly=false'
        driver.execute_script(t1)
        Mytool.findId(driver,"subs_time_s",st)
        time.sleep(1)
        driver.execute_script(t2)
        Mytool.findId(driver,"subs_time_s",ed)
#set address
    def set_Addr(self):
        driver=self.driver
        Mytool.findId(driver,"address_btn").click()
        ...
        Mytool.findClass(driver,"ok1").click()
        #获取输入文本框的值
        

#选择保证金货信用金挂牌时，选择品种等
#xpath路经应该是这种类型
    def goods_pick(self,i=0,j=0):
        """i 代表品种选择第i个
            j代表品名选择第j个  
        """
        driver=self.driver
        Mytool.findId(driver,"btn_add").click()

        elem=self.driver.find_element_by_xpath('//*[@id="addgoodsForm"]/div[1]/div[2]/div[1]/div[1]/div/div/div/div/div')
