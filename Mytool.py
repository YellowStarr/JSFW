...
def readExec(path):
  ...
  if(os.path.exists(path)):
    ...
    for i in range(1,rows):
      for j in range(cols):
        # key=ta.cell(i,j).value
        value=ta.cell(i,j).value
        # print key
        print value
        # setDict(key,value)
...
def findClass(dri,classname):
  ...
  return ele
def findClasses(dri,classname):
  ele=dri.find_elements_by_class_name(classname)
  return ele
...
def findLink(dri,link):
  ...
  return ele
def findLinks(dri,link):
  ele=dri.find_elements_by_link_text(link)
  return ele
...
def findTag(dri,tagname):
  ...
  return ele
def findTags(dri,tagname):
  eleDic=dri.find_elements_by_tag_name(tagname)
  return eleDic
